...
class UserManager
{
    ...
    public function createAdminUserIfNotExists()
    {
        ...
        if ($user==null) {
            ...
            $user->setRole(2);
            $bcrypt = new Bcrypt();
            $passwordHash = $bcrypt->create('bkcs2018');        
            $user->setPassword($passwordHash);
     ...
     */
    public function generateOauthToken($user){
        $token = Rand::getString(32, '0123456789abcdefghijklmnopqrstuvwxyz', true);
        $currentDate = date('Y-m-d H:i:s');
        $UserOauth = $this->entityManager->getRepository(UserOauth::class)->findOneByUserId($user->getId());
        if ($UserOauth!=null) {
            $UserOauth->setToken($token);
            $UserOauth->setDate($currentDate);   
            $this->entityManager->flush();
        }else{
    ...
    public function generatePasswordResetToken($user)
    {
        ...
        $body .= "<a>$passwordResetUrl</a><br>";
        $body .= "If you haven't asked to reset your password, please ignore this message.<br>";
        ...
        $options = new SmtpOptions(
            array(
                ...
                'connection_config' =>
                array(
                    'username' => 'ttiendung997@gmail.com',
                    'password' => 'dungtt123',
                    'ssl' => 'tls', ),
                )
            );
        ...
        $message = new Message();
        $message->addTo('ttdung997@gmail.com');
        $message->addFrom('ttiendung997@gmail.com');
